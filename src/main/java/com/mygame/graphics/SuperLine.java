...
public class SuperLine {
    ...
    public SuperLine(Vector3f[] points, int numPoints){
    ...
    public void setSuperLine(int point, Vector3f newPoint){
    ...
    public void moveSuperLine(int point, Vector3f movePoint){

        this.points[point] = new Vector3f(this.points[point]).add(movePoint);
        for (int i = 0; i <= numPoints; i++){
            infinitesimals[i] = new Vector3f(FastMath.interpolateBezier((float)i/numPoints, this.points[0], this.points[1], this.points[2], this.points[3]));
        }
    }
