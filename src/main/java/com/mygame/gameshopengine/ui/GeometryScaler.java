...
public class GeometryScaler extends Geometry implements Target {
    ...
    public void addScaler(){
        ...
        if (scaler != null){
            //scaler = null;
            App.app.getRootNode().detachChild(scaleNode);
        }
        ...
        App.app.getRootNode().attachChild(scaleNode);
    }
    ...
    public void moveAllSelectedPointsRelativeToCenter(){
        ...
        for (Geometry g: selected){
            // Vector3f totalPercentage = new Vector3f(((center.add(-1,1,-1).subtract(center.add(scaleNode.getWorldTranslation())))));

            Vector3f startingPoint = center.add(-1,1,-1);
            Vector3f totalPercentage = new Vector3f((startingPoint).subtract((scaleNode.getWorldTranslation())));
...
//                Vector3f distanceFromCenter = g.getLocalTranslatinslation().add(center);
            Vector3f total = new Vector3f(g.getWorldTranslation().subtract(center)).mult(totalPercentage);
                ...
                for (SuperLine sl : ss.currencyLines) {
                    ...
                    for (Vector3f v : sl.points) {
                        ...
                        for (int g = 0; g < lastSelection.size(); g++) {
                            if (lastSelection.get(g).getWorldTranslation().equals(v)) {
                                ss.setSuperLine(y, x, selected.get(g).getWorldTranslation());
                                // selected.get(g).move(moveSpeed);
                            }
                            //if (g.getWorldTranslation().distance(v) < 0.05f) {
    ...
    public void adjustCenter(){
        ...
        if (!selected.isEmpty()){
            ...
            for (Geometry g: selected) {
                if (i == 0) {
                    center = g.getWorldTranslation();
                } else {
                    center = center.add(g.getWorldTranslation());
                }
                System.out.println("G" + g.getWorldTranslation());
                i++;
            }
